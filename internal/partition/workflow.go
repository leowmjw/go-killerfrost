...
func LifeCycleWorkflow(ctx workflow.Context) error {
	...
	workflow.Go(ctx, func(ctx workflow.Context) {
		for {
			// Need heartbeat here every 50s? fails after 60s . why?
...
func checkerLoop(ctx workflow.Context, tt *TrackedTable) {
	if tt.IsTest {
		fmt.Println("ISTEST: Minor unit is ...")
	}
	...
	// Slice Projection ..
	tt.getSliceProjection(timeNow, 3)
	// After; loop through it
	for _, v := range tt.Ranges {
		spew.Dump(v)
		// If got expected actions; then call it as activity ..
	}
	// Check knowledge against current state; add more that are missing?
	// Above is the plan; signal need approval?
	// or just apply it what ismissing
